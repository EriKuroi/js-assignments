...
function roundToPowerOfTen(num, pow) {
    ...
    let rounder = pow;
    let rounded = num.toString().split('').reduceRight((acc, el, index, array) => {
        
        let elRound = array[index+1] > 4 ?  (Number(el) + 1) : el;
        let newEl = rounder >= 0? elRound: el;
        acc = rounder > 0 ? acc + 0 : acc + newEl;
        rounder -=1;
        return acc;
    }, '');
    let result = rounded.split('').reverse().join('');
    if ((''+num).split('').shift() === result.split('').shift()){
