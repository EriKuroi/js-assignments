...
function encodeToRot13(str) {
    function setBorders(charcode){
        return charcode < 91 
        ? "090"
        :"122";
    }
    function shift(letter, index, string){
        let charcode = string.charCodeAt(index);
        console.log(charcode);
        const shifted = charcode+13;
        console.log("shift",charcode);
        console.log("-bord",charcode);

        const border = setBorders(charcode)
        return shifted <= border
        ? String.fromCharCode(shifted)  
        : String.fromCharCode(shifted-border);
    }
    
    str.replace(/[A-Za-z]/, shift);
}
